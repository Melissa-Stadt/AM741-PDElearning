...
model_names = ['finite_differences','bisplines','NCV_bisplines','nn']
print_names = ['FD','LCVSP','LNCVSP','ANN']
skip = 20
threshold = 0.0001
threshold2 = -1
threshold3 = -0.01
...
for data_name in data_names:
    ...
    for ind in inds:
        ...
        for model_name in model_names:
            ...
            U_t_mse = np.mean(np.square((U_t_true - U_t_pred)[np.abs(U_t_true>threshold2)] / \
                                         U_t_true[np.abs(U_t_true>threshold2)]))
            U_x_mse = np.mean(np.square((U_x_true - U_x_pred)[np.abs(U_x_true>threshold)] / \
                                         U_x_true[np.abs(U_x_true>threshold)]))
            U_xx_mse = np.mean(np.square((U_xx_true - U_xx_pred)[np.abs(U_xx_true>threshold3)] / \
                                          U_xx_true[np.abs(U_xx_true>threshold3)]))
